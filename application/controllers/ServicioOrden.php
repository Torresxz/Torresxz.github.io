 ...
 *
 */
class ServicioOrden extends CI_Controller
{
    ...
    {
        $data['orden'] = $this->{$this->model}->listar_usuarios();

        foreach ($data['orden'] as $key => $usuario) {
        ...
        $this->load->view($this->views.'ordenserv');
    }
    // -------------------------------------------------------------------
